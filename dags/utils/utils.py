    ...
    def retry_wrapper(*args, **kwargs):
        ...
        while attempts < retries:
            try:
                ...
                logging.error(
                    f"Retrying from attempt number {attempts} because of error in API or data is not in json format"
                )
                attempts += 1
        logging.exception("Exceed max retry num: {} failed".format(attempts))
        raise Exception(
            "The request from API is not working or the data its not in json format"
        )
...
def get_max_reload(connection_string, schema):
    ...
    if (
        pd.read_sql(
            text("""select max(reload_id) from metadata_load;"""), con=conn
        ).values[0][0]
        is None
    ):
        reload_id = 1
...
def get_table_station_surrogate(connection_string, schema):
    try:
        ...
        data = pd.read_sql(
            text(
                """select pk_surrogate_station_info,station_id  from  eco_bikes.station_info_eco_bikes where is_active = 1"""
            ),
            conn,
        )
    except Exception as e:
...
def transform_scd_station_info(path_parquet, string_connection, schema):
    logging.info(
        f"Reading parquet file: {path_parquet['station_info_eco_bikes']}.parquet in {path_parquet['station_info_eco_bikes']}"
    )
    data_src = pd.read_parquet(path_parquet["station_info_eco_bikes"])
    conn = create_engine(
        string_connection, connect_args={"options": f"-csearch_path={schema}"}
    )
    data_target = pd.read_sql_query(
        "SELECT * FROM eco_bikes.station_info_eco_bikes", con=conn
    )
    ...
    df_excluding_new = pd.concat(
        [df_merge_col, df_new_records], axis=0
    ).drop_duplicates(keep=False)
    df_new_records_final = df_new_records.copy()
    df_new_records_final = df_new_records_final[
        [
            "station_id",
            "station_name_x",
    ...
    df_scd2_records = df_excluding_new[
        (df_excluding_new["station_name_x"] != df_excluding_new["station_name_y"])
        | (
            df_excluding_new["physical_configuration_x"]
            != df_excluding_new["physical_configuration_y"]
        )
        ...
        | (
            df_excluding_new["is_charging_station_x"]
            != df_excluding_new["is_charging_station_y"]
        )
        | (
            df_excluding_new["nearby_distance_x"]
            != df_excluding_new["nearby_distance_y"]
        )
        | (
            df_excluding_new["_ride_code_support_x"]
            != df_excluding_new["_ride_code_support_y"]
        )
    ]
    df_scd2_records_final_replace = df_scd2_records.copy()
    df_scd2_records_final_replace = df_scd2_records_final_replace[
        [
            "station_id",
            "station_name_y",
    ...
    df_scd2_records_final_replace.columns = [
        "station_id",
        "station_name",
        "physical_configuration",
        "lat",
    ...
    df_scd2_records_final_replace["start_date"] = df_scd2_records_final_replace[
        "start_date"
    ].astype(str)
    ...
    df_scd2_records_final_append = df_scd2_records_final_append[
        [
            "station_id",
            "station_name_x",
            "physical_configuration_x",
    ...
    df_scd2_records_final_append.columns = [
        "station_id",
        "station_name",
        "physical_configuration",
        "lat",
...
def gcp_create_dataset_and_tables(
    credentials_path, project_id, dataset_id, sql_file_path
):
    ...
    for filename in os.listdir(sql_file_path):
        if filename.endswith(".sql"):
            ...
            logging.info(
                f"Tables created in BigQuery dataset '{project_id}.{dataset_id}' from SQL file '{sql_file_path}'"
            )
...
def df_to_gcbq(df, table_name, path_to_json_credentials, project_id, method):
    ...
    try:
        ...
        df.to_gbq(
            table_name, project_id=project_id, if_exists=method, credentials=credentials
        )
    except Exception as e:
        logging.error(f"Connection error {e}")
        raise Exception(f"Connection error {e}")
...
def gcp_add_surrogate_ket_station_status(df, credentials, project_id, dataset_id):
    data_surrogate = gcp_get_table_station_surrogate(
        credentials, project_id, dataset_id
    )
    df = df.merge(data_surrogate, on="station_id", how="inner")
    return df
...
def gcp_transform_scd_station_info(
    path_parquet, credentials, project_id, dataset_id, bucket_name
):
    logging.info(
        f"Reading parquet file: {path_parquet['station_info_eco_bikes']}.parquet in {path_parquet['station_info_eco_bikes']}"
    )
    data_src = gcp_read_parquet(
        path_parquet["station_info_eco_bikes"], project_id, credentials, bucket_name
    )
    credentials = service_account.Credentials.from_service_account_file(credentials)
    sql_target = f"SELECT * FROM {dataset_id}.station_info_eco_bikes"
    data_target = pd.read_gbq(
        sql_target, project_id=project_id, credentials=credentials
    )
    datetime_now = pd.to_datetime(dt.datetime.now(), format="%Y-%m-%d %HH:%MM:%SS")
    data_target_current = data_target[(data_target["is_active"] == 1)]
    df_merge_col = pd.merge(data_src, data_target_current, on="station_id", how="left")
    ...
    df_scd2_records_final_append["is_active"] = 1
    df_scd2_records_final_replace["pk_surrogate_station_info"] = (
        df_scd2_records_final_replace["start_date"].dt.strftime("%Y%m%d%H%M%S")
        + df_scd2_records_final_replace["station_id"]
    )
    df_new_records_final["pk_surrogate_station_info"] = (
        df_new_records_final["start_date"].dt.strftime("%Y%m%d%H%M%S")
        + df_new_records_final["station_id"]
    )
    df_scd2_records_final_append["pk_surrogate_station_info"] = (
        df_scd2_records_final_append["start_date"].dt.strftime("%Y%m%d%H%M%S")
        + df_scd2_records_final_append["station_id"]
    )
    df_scd2_records_final_replace[
        "pk_surrogate_station_info"
    ] = df_scd2_records_final_replace["pk_surrogate_station_info"].astype(int)
    df_new_records_final["pk_surrogate_station_info"] = df_new_records_final[
        "pk_surrogate_station_info"
    ].astype(int)
    df_scd2_records_final_append[
        "pk_surrogate_station_info"
    ] = df_scd2_records_final_append["pk_surrogate_station_info"].astype(int)
    return (
        df_scd2_records_final_replace,
        df_new_records_final,
        df_scd2_records_final_append,
...
def gcp_save_json(request_json, filename, path_storage_credentials, bucket_name):
    logging.info(
        f"Saving request in json format in GCP BUCKET: {bucket_name}/{filename}.json"
    )
...
def gcp_load_to_parquet(
    df, filename, project_id, path_storage_credentials, bucket_name
):
    logging.info(f"Loading parquet file from {bucket_name}/parquet/{filename}")
    fs = gcsfs.GCSFileSystem(project=project_id, token=path_storage_credentials)
...
def gcp_create_bucket_and_folders(
    project_id, bucket_name, folder_names, credentials_path
):
    ...
    try:
        ...
        for folder_name in folder_names:
            ...
            blob.upload_from_string("")
            logging.info(
                f"GCP {project_id} - Bucket {bucket.name} - creating Folder {folder_name} created inside {bucket.name}"
            )
