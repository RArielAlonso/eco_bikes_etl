...
def gcp_create_bucket():
    gcp_create_bucket_and_folders(GCP_PROJECT_ID, GCP_BUCKET_NAME, ['json', 'parquet'], GCP_STORAGE_JSON_CREDENTIALS)
...
def gcp_load_dim_date(path_parquet):
    logging.info(f"Reading parquet file: {path_parquet['dim_date']} in {path_parquet['dim_date']}")
    data = gcp_read_parquet(path_parquet['dim_date'], GCP_PROJECT_ID, GCP_STORAGE_JSON_CREDENTIALS, GCP_BUCKET_NAME)
...
def load_to_gcp_append(path_parquet_files):
    for key, value in path_parquet_files.items():
        ...
        data = gcp_read_parquet(value, GCP_PROJECT_ID, GCP_STORAGE_JSON_CREDENTIALS, GCP_BUCKET_NAME)
