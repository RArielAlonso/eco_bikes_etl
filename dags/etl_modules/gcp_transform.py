...
def gcp_transform_weather(path_jsons):
    ...
    data_clouds = pd.json_normalize(gcp_load_json(path_jsons['weather'], GCP_STORAGE_JSON_CREDENTIALS, GCP_BUCKET_NAME)['weather'])
    data_temperature = pd.json_normalize(gcp_load_json(path_jsons['weather'], GCP_STORAGE_JSON_CREDENTIALS, GCP_BUCKET_NAME)['main'])
    data_date = pd.json_normalize(gcp_load_json(path_jsons['weather'], GCP_STORAGE_JSON_CREDENTIALS, GCP_BUCKET_NAME))
...
def gcp_transform_system_info(path_jsons):
    ...
    data_system_info = pd.json_normalize(gcp_load_json(path_jsons['system_info_eco_bikes'], GCP_STORAGE_JSON_CREDENTIALS, GCP_BUCKET_NAME))
...
def gcp_transform_station_status(path_jsons):
    ...
    data_station_status = pd.DataFrame.from_dict(pd.json_normalize(gcp_load_json(path_jsons['station_status_eco_bikes'],
                                                                                 GCP_STORAGE_JSON_CREDENTIALS, GCP_BUCKET_NAME))['data.stations'][0])
...
def gcp_transform_station_info(path_jsons):
    ...
    data_station_info = pd.DataFrame.from_dict(pd.json_normalize(gcp_load_json(path_jsons['station_info_eco_bikes'],
                                                                 GCP_STORAGE_JSON_CREDENTIALS, GCP_BUCKET_NAME))['data.stations'][0])
